     ...
     //return # of points moved
{
  ...
  while (loopTimes < chain_length){
    ...
    if(boundary_points != 1)
      {
	for (j=0; j<nvtxs; j++)
	  minchange = onePoint_move(graph, nparts, sum, squared_sum, w, self_sim, linearTerm, j);
      }
    else
      {    
	for (ii=0; ii<nbnd; ii++){
	  minchange = onePoint_move(graph, nparts, sum, squared_sum, w, self_sim, linearTerm, bndind[ii]);
